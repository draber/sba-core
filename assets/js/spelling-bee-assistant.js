(function () {
	...
	class Widget {
	    getState() {
	        const stored = prefStore.get(this.key);
	        return typeof stored !== 'undefined' ? stored : this.defaultState;
	    }
	    setState(state) {
	        if (this.canChangeState) {
	            prefStore.set(this.key, state);
	        }
	        return this;
	    }
	    toggle(state) {
	    ...
	    constructor(app, key) {
	        ...
	        this.parts = {
	            ...
	            footer: src.div({
	                ...
	                content: [
	                    src.a({
	                        content: confStore.get('label'),
	                        attributes: {
	                            href: confStore.get('url'),
	                            target: prefix()
	                        }
	                    })
	                ]
	...
	class Score extends TablePane {
	    getData() {
	        ...
	        return [
	            ['', '✓', '?', '∑'],
	            ['W'].concat(keys.map(key => dataStore.getCount(key))),
	            ['P'].concat(keys.map(key => dataStore.getPoints(key)))
	        ];
	    }
	    constructor(app) {
	...
	class SpillTheBeans extends Plugin {
	    run(evt) {
	        ...
	        if (!evt.detail) {
	            emoji = '😐';
	        } else if (!dataStore.get('remainders').filter(term => term.startsWith(evt.detail)).length) {
	            emoji = '🙁';
	        }
	...
	class LetterCount extends TablePane {
	    getData() {
	        ...
	        dataStore.get('answers').forEach(term => {
	            ...
	            if (dataStore.get('foundTerms').includes(term)) {
	...
	class FirstLetter$1 extends TablePane {
	    getData() {
	        const letters = {};
	...
	class YourProgress extends TablePane {
	    display() {
	        const points = dataStore.getPoints('foundTerms');
	        const max = dataStore.getPoints('answers');
	    ...
	    getData() {
	        const maxPoints = dataStore.getPoints('answers');
	        return [
	            ['Beginner', 0],
	            ['Good Start', 2],
	            ['Moving Up', 5],
	    ...
	    getCurrentTier() {
	        return this.getData().filter(entry => entry[1] <= dataStore.getPoints('foundTerms')).pop()[1];
	    }
	    getPointsToNextTier() {
	        const remainders = this.getData().filter(entry => entry[1] > dataStore.getPoints('foundTerms')).shift();
	        return remainders ? remainders[1] : null;
	    }
	    constructor(app) {
	        super(app, 'Your Progress', 'The number of points required for each level', {
	            cssMarkers: {
	                completed: rowData => rowData[1] < dataStore.getPoints('foundTerms') && rowData[1] !== this.getCurrentTier(),
	...
	class Community extends Plugin {
	    hasGeniusNo4Letters() {
	        const maxPoints = dataStore.getPoints('answers');
	        const no4LetterPoints = maxPoints - dataStore.get('answers').filter(term => term.length === 4).length;
	        return no4LetterPoints >= Math.round(70 / 100 * maxPoints);
	    }
	    getPerfectPangramCount() {
	        return dataStore.get('pangrams').filter(term => term.length === 7).length;
	    }
	    hasBingo() {
	        return Array.from(new Set(dataStore.get('answers').map(term => term.charAt(0)))).length === 7;
	    }
	    nytCommunity() {
	        const date = dataStore.get('printDate');
	    ...
	    constructor(app) {
	        ...
	        const pane = src.ul({
	            ...
	            content: src.li({
	                ...
	                content: `Show ${confStore.get('title')}`
	            })
	        });
	        this.ui = src.div({
	            events: {
	        ...
	        app.on(prefix('pluginsReady'), evt => {
	            ...
	            pane.append(src.li({
	                ...
	                content: src.a({
	                    content: [
	                        iconCoffee,
	                        confStore.get('kofi.text'),
	                    ],
	                    attributes: {
	                        href: confStore.get('kofi.url'),
	                        target: prefix()
	                    }
	                })
	            }));
	...
	class Grid extends TablePane {
	    ...
	    getData() {
	        const foundTerms = dataStore.get('foundTerms');
	        const allTerms = dataStore.get('answers');
